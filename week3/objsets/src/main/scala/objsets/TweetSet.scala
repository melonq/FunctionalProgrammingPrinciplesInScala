...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def maxRetweet(x: Tweet, y: Tweet) = {
    ...
    (left, right) match {
      ...
      case (l: NonEmpty, r: NonEmpty) => maxRetweet(maxRetweet(l.mostRetweeted, r.mostRetweeted), elem)
    }
  }
}
...
object GoogleVsApple {
  ...
  def getTweets(keywords: List[String]): TweetSet = keywords.map(x => {
    TweetReader.allTweets.filter(_.text.contains(x))
  }).reduce(_ union _)
